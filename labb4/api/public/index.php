...
$router->add('/post', PostController::class)
    ->middleware([AuthMiddleware::class]);
$router->group(function (Router $publicRouter) {
    $publicRouter->add('/post/public/user/*', PostController::class)
        ->classMethod('getByUser');
    $publicRouter->add('/post/public', PostController::class)
        ->classMethod('getAll');
    $publicRouter->add('/auth', AuthController::class);
    $publicRouter->add('/user/public/*', UserController::class)
        ->classMethod('getUserProfileInfo');
